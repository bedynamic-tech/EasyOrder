...
register_uninstall_hook( __FILE__, 'eo_uninstall' );
function eo_uninstall(): void {
    delete_option( 'easyorder_recipients' );
    delete_option( 'easyorder_send_confirmation' );
    delete_option( 'easyorder_confirmation_pricing' );
    ...
    foreach ( [ EO_TAX, EO_STRAIN ] as $taxonomy ) {
...
function eo_action_links( array $links ): array {
    array_unshift( $links, '<a href="' . esc_url( admin_url( 'options-general.php?page=easyorder-settings' ) ) . '">Settings</a>' );
    return $links;
}
...
function eo_register_settings_page(): void {
...
function eo_register_settings(): void {
...
function eo_sanitize_recipients( string $value ): string {
    $emails = array_filter( array_map( 'trim', explode( ',', $value ) ), 'is_email' );
    return implode( ',', $emails );
}

function eo_render_settings_page(): void {
    ?>
    <div class="wrap">
...
function eo_save_meta( int $post_id ): void {
    if (
    ...
    wp_set_post_terms( $post_id, ! empty( $_POST['eo_types'] )      ? [ intval( $_POST['eo_types'] ) ]      : [], EO_STRAIN );
    wp_set_post_terms( $post_id, ! empty( $_POST['eo_categories'] ) ? [ intval( $_POST['eo_categories'] ) ] : [], EO_TAX );
}

// --- Assets ---

add_action( 'wp_enqueue_scripts', 'eo_enqueue_assets' );
function eo_enqueue_assets(): void {
    global $post;
    if ( ! is_a( $post, 'WP_Post' ) || ! has_shortcode( $post->post_content, 'easyorder_form' ) ) {
        return;
    }

    wp_register_style( 'easyorder', false, [], EO_VERSION );
    wp_add_inline_style( 'easyorder', eo_get_css() );
    wp_enqueue_style( 'easyorder' );

    wp_register_script( 'easyorder', false, [], EO_VERSION, true );
    wp_add_inline_script( 'easyorder', 'var eoData=' . wp_json_encode( [ 'ajaxUrl' => admin_url( 'admin-ajax.php' ) ] ) . ';', 'before' );
    wp_add_inline_script( 'easyorder', eo_get_js() );
    wp_enqueue_script( 'easyorder' );
}

function eo_get_css(): string {
    return <<<'CSS'
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function applyFilters() {
        ...
        tbody.querySelectorAll('tr.eo-row').forEach(function (row) {
            ...
            const matchType  = ! strain || row.dataset.type     === strain;
            const matchCat   = ! cat    || row.dataset.category === cat;
            const matchStock = ! stock  || parseInt(row.dataset.stock, 10) > 0;
            const show       = matchText && matchType && matchCat && matchStock;
    ...
    // --- Sorting (by named data-sort attribute, not column index) ---
    document.querySelectorAll('.eo-sortable').forEach(function (th) {
        th.addEventListener('click', function () {
            ...
            document.querySelectorAll('.eo-sortable').forEach(function (h) { h.classList.remove('asc', 'desc'); });
            th.classList.add(dir);

            const rows = Array.from(tbody.querySelectorAll('tr.eo-row'));
            rows.sort(function (a, b) {
                const aCell = a.querySelector('[data-sort="' + sortKey + '"]');
                const bCell = b.querySelector('[data-sort="' + sortKey + '"]');
                const aVal  = aCell ? ( aCell.dataset.value !== undefined ? aCell.dataset.value : aCell.textContent.trim() ) : '';
                const bVal  = bCell ? ( bCell.dataset.value !== undefined ? bCell.dataset.value : bCell.textContent.trim() ) : '';
    ...
    form.addEventListener('submit', function (e) {
        ...
        form.querySelectorAll('.eo-qty').forEach(function (input) {
            ...
            if ( qty > 0 ) {
                items.push({ id: input.dataset.id, qty: qty });
            }
        });
        ...
        fetch(eoData.ajaxUrl, { method: 'POST', body: data })
            ...
            .then(function (res) {
                ...
                if ( res.success ) {
                    ...
                    form.reset();
                    form.querySelectorAll('.eo-qty').forEach(function (i) { i.value = 0; });
...
function eo_redirect_if_logged_out(): void {
...
function eo_render_shortcode(): string {
    ...
    foreach ( $products as $p ) {
        if ( get_post_meta( $p->ID, '_eo_sku', true ) ) {
            $has_sku = true;
            break;
        }
    }
            ...
            <table class="eo-table">
                <thead>
                    <tr>
                        ...
                        <th class="eo-sortable" data-sort="name"     data-type="string">Product  <span class="eo-sort-icon"></span></th>
                        <?php if ( $has_type ) : ?><th class="eo-sortable" data-sort="type"     data-type="string">Type     <span class="eo-sort-icon"></span></th><?php endif; ?>
                        <?php if ( $has_cat )  : ?><th class="eo-sortable" data-sort="category" data-type="string">Category <span class="eo-sort-icon"></span></th><?php endif; ?>
                        <th class="eo-sortable" data-sort="price"    data-type="number">Price    <span class="eo-sort-icon"></span></th>
                        <th class="eo-sortable" data-sort="stock"    data-type="number">In Stock <span class="eo-sort-icon"></span></th>
                        <th>Qty</th>
                    </tr>
                </thead>
                <tbody id="eo-tbody">
                    ...
                    >
                        ...
                        <td data-label="Product"  data-sort="name"     class="eo-product-name"><?php echo esc_html( $product->post_title ); ?></td>
                        <?php if ( $has_type ) : ?><td data-label="Type"     data-sort="type"    ><?php echo esc_html( $strain_name ?: '---' ); ?></td><?php endif; ?>
                        <?php if ( $has_cat )  : ?><td data-label="Category" data-sort="category"><?php echo esc_html( $cat_name    ?: '---' ); ?></td><?php endif; ?>
                        <td data-label="Price"    data-sort="price"    data-value="<?php echo esc_attr( $price ?: 0 ); ?>"><?php echo $price ? '$' . number_format( (float) $price, 2 ) : '---'; ?></td>
                        <td data-label="In Stock" data-sort="stock"    data-value="<?php echo esc_attr( $stock ); ?>">
                            <?php if ( $stock > 0 ) : ?>
...
/**
 * Resolve raw submitted items into enriched data, validating post existence and type.
 * Performs all DB queries in one pass so the AJAX handler and email builder share the same data.
 *
 * @param array $raw_items  Raw items from JSON decode, each with 'id' and 'qty'.
 * @return array            Enriched items, each with post, qty, sku, price, stock, strain keys.
 */
function eo_resolve_items( array $raw_items ): array {
    $resolved = [];
    ...
    foreach ( $raw_items as $item ) {
        ...
        $resolved[] = [
            'post'   => $post,
            'qty'    => $qty,
            'sku'    => get_post_meta( $post_id, '_eo_sku',   true ),
            'price'  => get_post_meta( $post_id, '_eo_price', true ),
            'stock'  => (int) get_post_meta( $post_id, '_eo_stock', true ),
            'strain' => ( $type_terms && ! is_wp_error( $type_terms ) ) ? $type_terms[0]->name : '',
        ];
    }
...
/**
 * Build HTML <tr> rows for an order email from pre-resolved item data.
 *
 * @param array $resolved   Items from eo_resolve_items().
 * @param bool  $show_price Whether to include price and line-total columns.
 * @return array            { rows: string, total: float }
 */
function eo_build_order_rows( array $resolved, bool $show_price ): array {
    $rows  = '';
    $total = 0.0;

    foreach ( $resolved as $i => $item ) {
        $name   = esc_html( $item['post']->post_title );
...
function eo_handle_submission(): void {
    if ( ! is_user_logged_in() ) {
    ...
    // Rate-limit: one submission per user per 15 seconds.
    $rate_key = 'eo_rate_' . get_current_user_id();
    if ( get_transient( $rate_key ) ) {
        wp_send_json_error( 'Please wait a moment before submitting another order.' );
    }
    ...
    // Resolve items (single DB pass shared by validation, stock decrement, and both emails).
    $resolved = eo_resolve_items( $raw_items );
    ...
    foreach ( $resolved as $item ) {
        if ( $item['qty'] > $item['stock'] ) {
            wp_send_json_error( 'Requested quantity for "' . esc_html( $item['post']->post_title ) . '" exceeds available stock.' );
        }
    }

    // Lock rate limit before touching stock or sending mail.
    set_transient( $rate_key, 1, 15 );

    // Decrement stock.
    foreach ( $resolved as $item ) {
        update_post_meta( $item['post']->ID, '_eo_stock', (string) max( 0, $item['stock'] - $item['qty'] ) );
    }
    ...
    $admin_result = eo_build_order_rows( $resolved, true );
...
<div class='email-wrap'>
  ...
  <div class='email-footer'>
    ...
    $confirm_result = eo_build_order_rows( $resolved, $confirmation_pricing );
...
<div class='email-wrap'>
  ...
  <div class='email-footer'>
    ...
    $confirm_sent = wp_mail( $sender_email_hdr, 'Your Order has been Received', $confirm_body, [ 'Content-Type: text/html; charset=UTF-8' ] );
    if ( ! $confirm_sent ) {
